...
void PinkyAI::normalAI(std::shared_ptr<Ghost> ghost, std::shared_ptr<PacmanMap> map)
{
	...
	else if (map->getAtXY(start.x, start.y) == FieldType::CROSSROAD || map->getAtXY(start.x, start.y) == FieldType::PACMAN_ONLY_CROSSROAD)
	{
		...
		else if (destinationSwitch.get_ticks() / 1000 <= Constants::minScatterTime + scatterTime || level >= Constants::breakPointLevel)
		{
			...
			{
			case eDirection::DIRECTION_UP: finish.y -= 4; break;
			case eDirection::DIRECTION_DOWN: finish.y += 4; break;
			case eDirection::DIRECTION_LEFT: finish.x -= 4; break;
			case eDirection::DIRECTION_RIGHT: finish.x += 4; break;
				default: break;
			}
