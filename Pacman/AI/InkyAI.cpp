...
void InkyAI::normalAI(std::shared_ptr<Ghost> ghost, std::shared_ptr<PacmanMap> map)
{
	SDL_Rect destPosition = toFollow->getPosition(), position = ghost->getPosition();
	eDirection direction = ghost->getDirection(), followDirection = toFollow->getDirection();
	SDL_Rect secondGhostPosition = followGhost->getPosition();
	...
	if (start.x == 6 && start.y == 7)
	{
		direction = eDirection::DIRECTION_RIGHT;
	}
	else if (start.x == 7 && start.y == 7)
	{
	...
	else if (map->getAtXY(start.x, start.y) == FieldType::CROSSROAD || map->getAtXY(start.x, start.y) == FieldType::PACMAN_ONLY_CROSSROAD)
	{
		...
		else if (destinationSwitch.get_ticks() / 1000 <= Constants::minScatterTime + scatterTime || level >= Constants::breakPointLevel)
		{
			...
			case eDirection::DIRECTION_UP:
				finish.y += 2 * (destPosition.y - 1 - secondGhostPosition.y); 
				finish.x += 2 * (destPosition.x - secondGhostPosition.x);
				break;
			...
			case eDirection::DIRECTION_LEFT:
				finish.y += 2 * (destPosition.y - secondGhostPosition.y);
				finish.x += 2 * (destPosition.x - 1 - secondGhostPosition.x);
				break;
