...
void ClydeAI::normalAI(std::shared_ptr<Ghost> ghost, std::shared_ptr<PacmanMap> map)
{
	...
	else if (map->getAtXY(start.x, start.y) == FieldType::CROSSROAD || map->getAtXY(start.x, start.y) == FieldType::PACMAN_ONLY_CROSSROAD)
	{
		...
		{
			if ((abs(start.x - finish.x) + abs(start.y - finish.y)) >= Constants::clydeTileProximity)
			{
